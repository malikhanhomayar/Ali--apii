...
// --- CONFIGURATION ---
const CREDENTIALS = {
    username: "Shoaibali315",
    password: "Maryamali315"
};

const BASE_URL = "http://145.239.130.45/ints";
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        ...
        try {
            ...
            const r3 = await axios.get(STATS_PAGE_URL, {
                headers: { ...COMMON_HEADERS, "Cookie": STATE.cookie, "Referer": `${BASE_URL}/agent/SMSCDRReports` },
                timeout: 15000
            });
            ...
            console.log(`✅ msi Login complete! sessKey: ${key}`);
            return; // success
...
router.get('/', async (req, res) => {
    ...
    const referer = `${BASE_URL}/agent/SMSCDRReports`;
    try {
        const raw = await fetchWithRelogin(
            () => {
                const ts = Date.now();
